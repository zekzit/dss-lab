...
def accumulate_node(node, factors, depth=0):
    ...
    if node.is_root:
        ...
        for entity in entities:
            ...
            accumulate_node(new_node, next_factors, depth + 1)
    else:
        # AnyNode(id=uuid.uuid4().hex, name="Leaf node", parent=node)
        rules = __fetch_assoc_rule(node.name, current_factor)
    ...
    for idx, nodes_in_level in enumerate(nodes_all_level):
        ...
        for node in nodes_in_level:
            ...
            if node.weight is not None:
                try:
                    node.score = node.weight * node.confidence
                except:
                    print("Node has no confidence")
            else:
                node.score = None
